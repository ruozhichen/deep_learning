...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("Linear-part"):
        linear_part = tf.reshape(embeddings, [-1, field_size * embedding_size])
        linear_weight = tf.get_variable(name='linear_weight', shape=[field_size * embedding_size, layers[0]],
                                        initializer=tf.glorot_normal_initializer())
        lz = tf.matmul(linear_part, linear_weight)  # None * D1

    # 优化版本，先计算一个目标向量，再计算inner或者outer
    with tf.variable_scope("PNN-layer"):
        product_out = []
        if FLAGS.model_type == 'Inner':
            inner_weight = tf.get_variable(name='innter_weight', shape=[layers[0], field_size], initializer=tf.glorot_normal_initializer())
            for i in range(layers[0]):
                weight_i = tf.expand_dims(inner_weight[i], axis=1)  # (F, 1)
                weighted_vector = tf.reduce_sum(tf.multiply(embeddings, weight_i), axis=1)  # (None, M)
                product_out.append(tf.reduce_sum(tf.square(weighted_vector), axis=1, keep_dims=True))  # (None, 1)
        elif FLAGS.model_type == 'Outer':
            outer_weight = tf.get_variable(name='outer_weight', shape=[layers[0], embedding_size, embedding_size], initializer=tf.glorot_normal_initializer())
            sum_vector = tf.reduce_sum(embeddings, axis=1)  # (None, M)
            p = tf.matmul(tf.expand_dims(sum_vector, axis=2), tf.expand_dims(sum_vector, axis=1))  # (None, M, M)
            for i in range(layers[0]):
                lpi = tf.multiply(p, tf.expand_dims(outer_weight[i], axis=0))  # (None, M, M)
                product_out.append(tf.expand_dims(tf.reduce_sum(lpi, axis=[1, 2]), axis=1))  # (None, 1)
        lp = tf.concat(product_out, axis=1)  # (None, D)
        bias = tf.get_variable(name='bias', shape=[layers[0]], initializer=tf.constant_initializer(0.0))
        deep_inputs = tf.nn.relu(lz + lp + bias)
